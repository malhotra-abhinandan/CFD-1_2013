///Program for Solution of Steady heat equation using  Point Successive Over Relaxation Method.
...
int main()
{
    ...
    //double a1=cos(pi/cols-1);
    //double a2=cos(pi/rows-1);
    //double a3=a1+(beta*beta*a2);
    //double a4=1+(beta*beta);
    //double a5=a3/a4;
    //double a=pow(a5,2);

    //w=(2-2*sqrt(1-a))/a;
    double w_opt=1.78;   ///Optimum Over relaxation factor
    ...
    cout<<"The Optimum Value of Over relaxation factor:"<<w_opt<<endl;
...
do
{
      ...
      for(int j=1;j<rows-1;j++)
      {
          ...
          {
              T[j][i]=((1-w_opt)*Tstar[j][i])+w_opt*A*(Tstar[j][i+1]+T[j][i-1]+(pow(beta,2))*(Tstar[j+1][i]+T[j-1][i]));
          }
      }
   ...
   k++; ///Increment for finding the counter or iteration number.
}while(errormax>1e-4);
